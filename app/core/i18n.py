...
TRANSLATIONS = {
    "es": {
        ...
        "errors.youtube_bot_protection": "YouTube ha pedido autenticacion para esta descarga. OmniDown intentara usar automaticamente la sesion de un navegador local si esta instancia corre en la misma maquina. Si corre en un servidor remoto, configure credenciales de YouTube en las variables de entorno YT_DLP_USERNAME y YT_DLP_PASSWORD con una cuenta dedicada.",
    ...
    "en": {
        ...
        "errors.youtube_bot_protection": "YouTube is requiring authentication for this download. OmniDown will try to use a local browser session automatically when this instance runs on the same machine. If it runs on a remote server, configure YouTube credentials in the environment variables YT_DLP_USERNAME and YT_DLP_PASSWORD with a dedicated account.",
